...
export async function RelicDetailView({ relicId, backRoute = "/lost-found", backLabel = "Back to Browse" }: RelicDetailViewProps) {
    const session = await getServerSession(authOptions);

    const result = await getRelicById(relicId);
                ...
                <Card className="bg-zinc-900/30 border-zinc-800/50 overflow-hidden">
                    <div className="flex flex-col lg:flex-row">
                        ...
                        <div className="relative w-full lg:w-1/3 min-h-[300px] lg:min-h-full bg-gradient-to-br from-zinc-900 to-zinc-950">
                            <ImageGallery images={relic.images} title={relic.title} className="p-4" />
                        </div>
                        ...
                        <div className="flex-1 p-6 lg:p-8 space-y-6">
                            ...
                            {/* Reporter & Date Row */}
                            <div className="grid md:grid-cols-2 gap-6 items-center pt-4 border-t border-zinc-800">
                                {/* Reporter Info */}
